...
#### **قوانين JSX اللي مافيش فيها هزار:**
...
### **Props: إزاي الـ Components تكلم بعضها**
...
## **5. التعمق في React (Events, Lists, Conditionals)**

### **أ) التعامل مع الـ Events**
سهلة جداً، شبه الـ HTML بس camelCase.
*   `onclick` -> `onClick`
*   `onsubmit` -> `onSubmit`
...
### **ب) Conditional Rendering (أظهر إيه امتى؟)**
...
## **7. Hooks للأداء العالي (Performance Optimization)**
...
## **8. الـ Custom Hooks: اصنع أدواتك 🛠️**
...
## **9. إدارة الـ State المتقدمة (Context & Reducers)**

### **أ) Context API: الحل لـ Prop Drilling**
بدل ما تقعد تباصي الـ Props من الجد للأب للابن للحفيد (Prop Drilling)، الـ Context بيعمل "سحابة" داتا فوق الـ Components. أي حد محتاج الداتا (زي الـ Theme أو User)، يمد ايده ياخدها.

1.  **Create:** `const ThemeContext = createContext('light');`
2.  **Provider:** `<ThemeContext.Provider value="dark"> ... </ThemeContext.Provider>`
3.  **Consume:** `const theme = useContext(ThemeContext);`

### **ب) `useReducer`: لما الـ State تتقل**
